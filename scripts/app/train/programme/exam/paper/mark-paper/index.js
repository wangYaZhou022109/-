                ...
                createQuestionTypes: function(paper) {
                    ...
                    if (paper.questions && paper.questions.length > 0) {
                        _.forEach(paper.questions, function(q) {
                            ...
                            obj[type].name = _.find(types, ['key', q.type.toString()]).value;
                            qq = {
                                id: q.id,
            ...
            mixin: {
                save: function(error) {
                    var errors = this.data || [],
                        temp = _.filter(errors, function(e) {
                ...
                check: function() {
                    var hasError = !_.every(this.data, function(e) {
                        return e.check;
                    });
                    if (hasError) {
                        this.app.message.error(_.map(_.filter(this.data, function(d) {
                            return !d.check;
                        }), function(dd) {
                            return '"' + dd.content + '":' + dd.error;
                        }).join(','));
                    }
                    return hasError;
                }
    ...
    callbacks: {
        init: function(payload) {
            ...
            return this.get(this.models.exam).then(function() {
                ...
                if (!data.name) {
                    data.name = exam.data.name;
                    data.isOnePageOneQuestion = exam.data.paperShowRule === 1;
    ...
    fn: function() {
        ...
        return this.Promise.create(function(resolve) {
            ...
            me.app.message.confirm(message, function() {
                return me.dispatch('save').then(function() {
